...
const SideRays = ({
  ...
  useEffect(() => {
    ...
    // Todo lo de aquí para abajo es SÍNCRONO -> no hay ventana de tiempo
    // en la que un cleanup pueda "perderse" por un remount rápido
    // (Fast Refresh / Strict Mode).
  ...
  gl_FragColor = color;
    ...
    const loop = t => {
      ...
      try {
        ...
        // contexto perdido u otro fallo: no seguimos pidiendo frames
      }
    };
    ...
    // Cleanup: se registra en el mismo tick, sin ventanas async donde
    // pueda perderse. Siempre se ejecuta cuando React desmonta o
    // vuelve a correr el efecto.
    return () => {
      ...
      try {
        ...
        // no-op: el contexto puede ya estar inválido
      }
